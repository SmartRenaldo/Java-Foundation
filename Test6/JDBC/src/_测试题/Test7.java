...
/**
 * 将userid 为Bbiri的users表和my_employees表的记录全部删除
 *
 * @author Gavin
 */
public class Test7 {
    public static void main(String[] args) throws Exception {
        ...
        String sql = "delete users,my_employees from users join my_employees " +
                "on users.userid = my_employees.userid and users.userid = 'Bbiri'";

        PreparedStatement statement = connection.prepareStatement(sql);
        int update = statement.executeUpdate();
        System.out.println(update > 0 ? "executed" : "failed");
    ...
    private static void queryTableUsers(Connection connection, String title) throws SQLException {
        ...
        while (tableSet.next()) {
            ...
            System.out.printf("%-15d%-15s%15-d\n", id, userid, department_id);
        }
